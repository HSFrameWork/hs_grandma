...
RegisterServerEvent('hs_grandma:server:checkfunds', function()
    local src = source
    ...
    if CheckBalance then -- Checks for money balance
        if balance >= Config.Cost then -- Checks to make sure player has enough funds
            TriggerClientEvent('hs_grandma:reviveplayer', source) -- Starts Progressbar for Reviving
        else
        ...
        TriggerClientEvent('hs_grandma:reviveplayer', source)
    end
end)
